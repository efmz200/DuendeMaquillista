...
function Mensajes(props) {
    ...
    useEffect(() => {
        var correoyr = user.correo
        // Hacer una solicitud para obtener las categorías desde la API
        fetch("http://localhost:3000/api/usuario/notificacionesUsr",{
            method: 'POST',
            body: JSON.stringify({correo:correoyr}),
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            }
            ...
            .then((data) => {
                console.log(correoyr)
                setContenidos(data.notificaciones);
